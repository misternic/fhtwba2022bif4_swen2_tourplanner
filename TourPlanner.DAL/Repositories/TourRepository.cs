...
public sealed class TourRepository : BaseRepository<Tour>
{
    ...
    private const string FIELDS = "id, name, description, \"from\", \"to\", transport_type, distance, estimated_duration";

    private Tour ReadAsTour(NpgsqlDataReader reader)
    {
        return new()
        {
            Id = reader.GetGuid(0),
    ...
    {
        var cmd = new NpgsqlCommand($"SELECT {FIELDS} FROM tours WHERE id=@id", Context.Connection, Context.Transaction);
    ...
    {
        var cmd = new NpgsqlCommand("INSERT INTO tours (id, name, description, \"from\", \"to\", transport_type, distance, estimated_duration) VALUES(@id, @name, @description, @from, @to, @transport_type, @distance, @estimated_duration)", Context.Connection, Context.Transaction);
    ...
    {
        var cmd = new NpgsqlCommand("UPDATE tours SET name=@name, description=@description, \"from\"=@from, \"to\"=@to, transport_type=@transport_type, distance=@distance, estimated_duration=@estimated_duration WHERE id=@id", Context.Connection, Context.Transaction);
        cmd.Parameters.AddWithValue("id", tour.Id);
        cmd.Parameters.AddWithValue("name", tour.Name);
        cmd.Parameters.AddWithValue("description", tour.Description);
        cmd.Parameters.AddWithValue("from", tour.From);
