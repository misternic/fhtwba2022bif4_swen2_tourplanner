...
public sealed class TourLogRepository : BaseRepository<TourLogDto>
{
    ...
    private const string FIELDS = "id, tour_id, date, difficulty, duration, rating, comment, created";
    ...
    {
        var cmd = new NpgsqlCommand("INSERT INTO logs (id, tour_id, date, difficulty, duration, rating, comment, created) VALUES(@id, @tour_id, @date, @difficulty, @duration, @rating, @comment, @created)", Context.Connection, Context.Transaction);
    ...
    {
        var cmd = new NpgsqlCommand("UPDATE logs SET date=@date, difficulty=@difficulty, duration=@duration, rating=@rating, comment=@comment, created=@created WHERE id=@id", Context.Connection, Context.Transaction);
        cmd.Parameters.AddWithValue("id", logDto.Id);
        cmd.Parameters.AddWithValue("date", logDto.Date);
        cmd.Parameters.AddWithValue("difficulty", (int) logDto.Difficulty);
        cmd.Parameters.AddWithValue("duration", logDto.Duration.TotalSeconds);
